...
const upsertCurrentPrice = async (priceData) => {
  const db = getPg();
  // 🚀 Added old_price to the destructuring block
  const { site, grade, price, old_price, last_updated_by } = priceData;
  ...
  const insertQuery = db(TABLE)
    .insert({
      ...
      grade,
      price,
      old_price: old_price !== undefined ? old_price : null, // 👈 Bound to insert statement
  ...
  // 🚀 Added EXCLUDED.old_price to the raw ON CONFLICT string update block
  const upsertQuery = `${insertQuery} ON CONFLICT (site, grade) DO UPDATE SET price = EXCLUDED.price, old_price = EXCLUDED.old_price, last_updated_by = EXCLUDED.last_updated_by, updated_at = CURRENT_TIMESTAMP RETURNING *;`;
