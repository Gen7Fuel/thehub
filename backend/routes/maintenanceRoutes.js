...
router.post('/:id/notify', async (req, res) => {
  try {
    ...
    // Email Lists
    const activeUsers = await User.find({ is_active: true }, 'email');
    const bccList = activeUsers.map(u => u.email);
    const ccList = ['mohammad@gen7fuel.com', 'kellie@gen7fuel.com'];
    const io = req.app.get("io");

    // Trigger Service
    await pushNotification({
      io,
      recipientEmails: ["daksh@gen7fuel.com", ...ccList], // Public recipients
      bccEmails: bccList, // Mass BCC list
      slug: "system-maintenance",
      subject: `🛠️ Scheduled System Maintenance: ${maintenance.name}`,
      fieldValues: {
        maintenanceName: maintenance.name,
        start: maintenance.scheduleStart.toISOString(),
        end: maintenance.scheduleClose.toISOString(),
    ...
    res.json({ message: `Maintenance notification broadcasted to ${bccList.length} users.` });
  } catch (err) {
...
router.post('/:id/cancel', async (req, res) => {
  try {
    ...
    // If notifications were previously sent, we notify everyone of the cancellation
    if (maintenance.notificationSent) {
      const activeUsers = await User.find({ is_active: true }, 'email');
      const bccList = activeUsers.map(u => u.email);
      const ccList = ['mohammad@gen7fuel.com', 'kellie@gen7fuel.com'];
      ...
      // 🧩 Call the updated pushNotification with BCC support
      await pushNotification({
        io,
        recipientEmails: ["daksh@gen7fuel.com", ...ccList],
        bccEmails: bccList, // Ensures all users get the Hub alert and BCC email
        slug: "system-maintenance-cancelled",
        subject: `🚫 CANCELLED: System Maintenance - ${maintenance.name}`,
        fieldValues: {
          maintenanceName: maintenance.name,
          start: maintenance.scheduleStart.toISOString(),
    ...
    res.json({ message: "Maintenance cancelled and users notified." });
  } catch (err) {
