...
const { invoiceProcessingQueue } = require("../queues/invoiceImageUploadQueue");
...
// Helper: Extract filename/CDN path string from ImageMetadata subdocuments
const formatInvoiceImages = (images) => {
  if (!Array.isArray(images)) return [];

  return images
    .map((img) => {
      if (typeof img === "string") return img; // Legacy plain string compatibility
      if (img && typeof img === "object") return img.name || ""; // Extract filename from subdocument
      return "";
    })
    .filter(Boolean);
};
...
router.get("/list", async (req, res) => {
  try {
    ...
    // 2. Date Range Filtering ('YYYY-MM-DD')
    if (fromDate || toDate) {
      query.invoiceDate = {};
      if (fromDate) query.invoiceDate.$gte = String(fromDate).trim();
      if (toDate) query.invoiceDate.$lte = String(toDate).trim();
    ...
    // Fetch invoices using .lean() for fast document mapping
    const invoices = await CsoInvoice.find(query)
      .populate("site", "stationName csoCode")
      .populate("submittedByMongoId", "name email")
      .sort({ invoiceDate: -1, createdAt: -1 })
...
router.get("/:id", async (req, res) => {
  try {
    ...
    const invoice = await CsoInvoice.findById(id)
      .populate("site", "stationName")
      .lean();
    ...
    return res.status(200).json({
      ...
      data: {
        ...
        checkNumber: invoice.checkNumber || "",
        totalCost: invoice.totalCost,
        // Pulls the `name` field out of each ImageMetadata object
        images: formatInvoiceImages(invoice.images),
        status: invoice.status,
      },
    });
...
router.post("/submit", async (req, res) => {
  try {
    ...
    // Persist shell document with empty images array first
    const newInvoice = new CsoInvoice({
      site: siteMongoId,
      siteCsoCode: targetCsoCode,
      submittedByMongoId: userMongoId,
    ...
    // 🚀 Dispatch CDN Upload & Blur Processing Queue
    await invoiceProcessingQueue.add(
      `process-images-${newInvoice._id}-${Date.now()}`,
      {
        invoiceId: newInvoice._id,
        invoiceImages,
        vendorCode,
    ...
    return res.json({
      success: true,
      message:
        "Your invoice has been submitted successfully! We are processing the upload in the background now.\nYou will be notified automatically as soon as it completes, or you can check the List page to view the current status.",
      invoiceId: newInvoice._id,
    });
  } catch (err) {
    ...
    return res.status(500).json({
      success: false,
      error: err.message || "Internal server error during submission sequence.",
    });
  }
});
...
router.put("/:id", async (req, res) => {
  try {
    ...
    // 🚀 Dispatch CDN Upload & Blur Processing Queue
    await invoiceProcessingQueue.add(
      `process-images-update-${existingInvoice._id}-${Date.now()}`,
      {
        invoiceId: existingInvoice._id,
        invoiceImages, // Contains mixed data: existing objects/filenames & new base64 strings
        vendorCode,
        docNumber,
        invoiceDate: dateStringOnly,
        existingImages: existingInvoice.images || [], // Pass current DB state to merge existing metadata
      },
      {
        attempts: 2,
        removeOnComplete: true,
    ...
    return res.json({
      success: true,
      message:
        "Your invoice has been updated successfully! Image processing is running in the background.",
      invoiceId: existingInvoice._id,
    });
  } catch (err) {
    console.error("❌ Update Route Error:", err);
