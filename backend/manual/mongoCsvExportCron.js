...
const { runMongoCsvExport } = require('../cron_jobs/mongoCsvExportCron');
...
async function run() {
  ...
  try {
    await connectDB();
    console.log(`🚀 Starting targeted export for collections...`);
    await runMongoCsvExport();
    console.log(`🚀 Completed export for collections`);
  } catch (err) {
    hadError = true;
