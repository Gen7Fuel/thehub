...
function generateSinglePdfReport(processedRecords, outputPdfPath, reportTitle) {
  ...
  function renderHeader(currentPage, totalPages) {
    ...
    doc.font('Helvetica-Bold').fontSize(8.5).text('FROM: ', 60, metaY, { continued: true })
       .font('Helvetica').text('JUNE 11, 2026');
...
function generatePdfReports() {
  ...
  const allProcessedRecords = rawRows.map(row => {
  ...
  // Define 4 report configurations
  const reportsToGenerate = [
    {
      targetFuelType: 'ALL',
      fileName: 'Admin Fee by Client - WAVERS OF BROKENHEAD - June 2026.pdf',
      title: 'ADMIN FEE BY CLIENT'
    },
    {
      targetFuelType: 'REGULAR',
      fileName: 'Admin Fee by Client - Regular - WAVERS OF BROKENHEAD - June 2026.pdf',
      title: 'ADMIN FEE BY CLIENT - REGULAR'
    },
    {
      targetFuelType: 'DIESEL',
      fileName: 'Admin Fee by Client - Diesel - WAVERS OF BROKENHEAD - June 2026.pdf',
      title: 'ADMIN FEE BY CLIENT - DIESEL'
    },
    {
      targetFuelType: 'PREMIUM',
      fileName: 'Admin Fee by Client - Premium - WAVERS OF BROKENHEAD - June 2026.pdf',
      title: 'ADMIN FEE BY CLIENT - PREMIUM'
    }
  ];

  // Generate each of the 4 reports
  reportsToGenerate.forEach(report => {
    const outputPdfPath = path.join(__dirname, report.fileName);

    let filteredRecords = allProcessedRecords;
    if (report.targetFuelType !== 'ALL') {
      filteredRecords = allProcessedRecords.filter(
        rec => rec.fuelType === report.targetFuelType
      );
    }

    // Create a deep copy to ensure clean sorting per report
    const recordsCopy = JSON.parse(JSON.stringify(filteredRecords));
    generateSinglePdfReport(recordsCopy, outputPdfPath, report.title);
  });
}
