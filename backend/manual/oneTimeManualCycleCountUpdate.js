...
async function syncInventoryFromExcel() {
  ...
  const workbook = xlsx.readFile('items_to_remove2.xlsx');
  ...
  const bulkOps = data.map((row) => {
    const upcExcel = String(row['gtin'] || '').trim();
    const stationExcel = String(row['site'] || '').trim();
    const descriptionExcel = String(row['name'] || '').trim();
    const categoryIdExcel = String(row['categoryNumber'] || '').trim();
    ...
    return {
      updateMany: {
        filter: {
          gtin: { $regex: new RegExp(`${upcExcel}`) }, // Direct match from Excel
          site: stationExcel,
          categoryNumber: categoryIdExcel,
          name: descriptionExcel, // Starts with first word
