...
// Updated InfoDialog to dynamically support Manitoba paths and titles
const InfoDialog = ({
  title,
  imageName,
  isManitoba
  ...
  // Determine asset base folder and image file prefix based on location
  const folder = isManitoba ? 'mb_lottery_reports' : 'lotto_max_reports';
  const finalImageName = isManitoba ? `mb_${imageName}` : imageName;
      ...
      <DialogContent className="max-w-4xl w-[90vw] overflow-y-auto max-h-[90vh]">
        ...
        <div className="flex flex-col items-center gap-6 py-4">
          ...
          <div className="text-sm md:text-base text-muted-foreground font-medium bg-slate-50 p-5 rounded-md border w-full">
            <ul className="list-none space-y-2">
              <li>
                <span className="text-red-600">●</span> <strong>Step 1:</strong> Locate the section marked by the <strong>Red Box</strong> on your Lottery Report.
              </li>
              <li>
                <span className="text-green-600">●</span> <strong>Step 2:</strong> Enter the corresponding value found inside the <strong>Green Box</strong>.
              </li>
    ...
    <div className="overflow-x-auto border rounded-md">
      <table className="min-w-full table-auto">
        ...
        <tbody>
          ...
          <tr className="border-t font-semibold">
            <td className="px-4 py-2 flex items-center">
              {isManitoba ? 'Total Online Sales' : 'Online Sales'}
              <InfoDialog
                title={isManitoba ? 'Total Online Sales' : 'Online Sales'}
                imageName="online_sales"
          ...
          <tr className="border-t bg-gray-50">
            <td className="px-4 py-2 pl-8 flex items-center">
              {isManitoba ? 'Credits (Total Online)' : 'Lotto Cancellations'}
              <InfoDialog
                title={isManitoba ? 'Credits (Total Online)' : 'Lotto Cancellations'}
                imageName="online_cancellations"
                isManitoba={isManitoba}
              />
            </td>
            <DataCell field="onlineCancellations" lottery={lottery} isReadOnly={isReadOnly} />
          ...
          {!isManitoba && (
            <>
              <tr className="border-t bg-gray-50">
                <td className="px-4 py-2 pl-8 flex items-center">
                  Lotto Discounts <InfoDialog title="Lotto Discounts" imageName="discounts" isManitoba={isManitoba} />
                </td>
                <DataCell field="onlineDiscounts" lottery={lottery} isReadOnly={isReadOnly} />
                <td colSpan={2} />
              </tr>
              ...
              <tr className="border-t bg-gray-50">
                <td className="px-4 py-2 pl-8 flex items-center">
                  Vouchers Redeemed <InfoDialog title="Vouchers Redeemed" imageName="vouchers_redeemed" isManitoba={isManitoba} />
                </td>
                <DataCell
                  field="vouchersRedeemed"
                  isPayoutCalc={true}
          ...
          <tr className="border-t font-semibold">
            <td className="px-4 py-2 flex items-center">
              {isManitoba ? 'Total Zing Sales' : 'Scratch Sales'} {/* Updated label for Manitoba */}
              {/* Info icon hidden here for Manitoba stores */}
              {!isManitoba && <InfoDialog title="Scratch Sales" imageName="scratch_sales" isManitoba={isManitoba} />}
            </td>
            {isManitoba ? (
          ...
          {/* Hidden Sub-rows under Scratch Sales for Manitoba */}
          {!isManitoba && (
            <>
              <tr className="border-t bg-gray-50">
                <td className="px-4 py-2 pl-8 flex items-center">
                  Scratch and Win (S&W) Free Tickets <InfoDialog title="SNW Free Tickets" imageName="scratch_ft_sales" isManitoba={isManitoba} />
                </td>
                <DataCell
                  field="scratchFreeTickets"
                  isPayoutCalc={true}
          ...
          {/* Free Tickets hidden for Manitoba */}
          {!isManitoba && (
            <tr className="border-t bg-gray-50">
              <td className="px-4 py-2 pl-8 flex items-center">
                On Demand (Online) Free Tickets <InfoDialog title="On Demand Free Tickets" imageName="online_ft_sales" isManitoba={isManitoba} />
              </td>
              <DataCell field="onDemandFreeTickets" lottery={lottery} isReadOnly={isReadOnly} />
              <td colSpan={2} />
            </tr>
          ...
          <tr className="border-t bg-gray-50">
            <td className="px-4 py-2 pl-8 flex items-center">
              {isManitoba ? 'Redemptions (Total Online)' : 'On Demand (Online) Cash Payout'}
              <InfoDialog
                title={isManitoba ? 'Redemptions (Total Online)' : 'On Demand Cash Payout'}
                imageName="online_payouts"
                isManitoba={isManitoba}
              />
            </td>
            <DataCell
              ...
              extraLogic={(val) => useFormStore.getState().setLotteryValues({
                onDemandCashPayout: val,
                payouts: val + (lottery.scratchCashPayout || 0) + (isManitoba ? 0 : (lottery.scratchFreeTickets || 0) + (lottery.vouchersRedeemed || 0))
              })}
              lottery={lottery}
              isReadOnly={isReadOnly}
            />
          ...
          {/* SCRATCH AND WIN CASH PAYOUT */}
          <tr className="border-t bg-gray-50">
            <td className="px-4 py-2 pl-8 flex items-center">
              {isManitoba ? 'Redemptions (Total Zing)' : 'Scratch and Win (S&W) Cash Payout'}
              <InfoDialog
                title={isManitoba ? 'Redemptions (Total Zing)' : 'SNW Cash Payout'}
                imageName="zing_payouts"
                isManitoba={isManitoba}
              />
            </td>
            <DataCell
              ...
              extraLogic={(val) => useFormStore.getState().setLotteryValues({
                scratchCashPayout: val,
                payouts: (lottery.onDemandCashPayout || 0) + val + (isManitoba ? 0 : (lottery.scratchFreeTickets || 0) + (lottery.vouchersRedeemed || 0))
              })}
              lottery={lottery}
              isReadOnly={isReadOnly}
            />
          ...
          {/* DATAWAVE SECTION (Hidden Entirely for Manitoba) */}
          {!isManitoba && (
            <>
              <tr className="border-t font-semibold">
                <td className="px-4 py-2">Datawave Value</td>
