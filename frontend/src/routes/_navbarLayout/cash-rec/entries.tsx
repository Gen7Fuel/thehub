              ...
              {rows.map(({ date, data }) => {
                ...
                return (
                  <tr className="border-t odd:bg-gray-50 even:bg-white" key={date}>
                    ...
                    <td className="px-2 py-2 text-right cursor-copy" onClick={copyCell}>{fmt2((data?.cashSummary?.totals.couponsAccepted ?? 0) + (data?.cashSummary?.totals.giftCertificates ?? 0) + (data?.cashSummary?.totals.cashOffCoupons ?? 0) + (data?.cashSummary?.totals.gasolineCoupons ?? 0) + (data?.cashSummary?.totals.otherCoupons ?? 0))}</td>
