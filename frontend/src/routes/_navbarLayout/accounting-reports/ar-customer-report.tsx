...
export const Route = createFileRoute(
  "/_navbarLayout/accounting-reports/ar-customer-report"
)({
...
function RouteComponent() {
  ...
  // Single toggle state: Include A/R Paid Report
  const [includeArPaidReport, setIncludeArPaidReport] = useState<boolean>(true);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setLoading(true);
      setStatusMessage("Requesting A/R Customer reports...");

      const token = localStorage.getItem("token");

      const response = await axios.post(
        "/api/accounting-reports/ar-customer-report",
        {
          stationName: site,
          startDate: startDateStr,
          endDate: endDateStr,
          includeArPaidReport: includeArPaidReport,
        },
        {
          headers: {
            ...
            "X-Required-Permission":
              "accounting.accountingReports.arCustomerReport",
          },
        }
      );
      ...
      setStatusMessage(
        response.data?.message ||
          `Report generated! The report will be emailed directly to ${targetEmail} shortly.`
      );
    } catch (err: any) {
      console.error("Error generating A/R report:", err);
    ...
    <div className="p-6 max-w-5xl mx-auto space-y-6">
      ...
      <div className="flex items-center justify-between pb-4 border-b border-slate-200 dark:border-zinc-800">
        <div>
          ...
          <p className="text-sm text-slate-500 dark:text-zinc-400">
            Generate and dispatch Accounts Receivable reports aggregated by customer directly to your email.
          </p>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-2xl p-6 shadow-sm">
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          {/* Toggle Option for Include A/R Paid Report */}
          <div className="flex items-center justify-between pt-4 border-t border-slate-100 dark:border-zinc-800/80">
            <div className="flex items-center gap-3">
              ...
              <div>
                <label
                  htmlFor="ar-paid-toggle"
                  className="text-sm font-semibold text-slate-800 dark:text-zinc-200 cursor-pointer"
                >
                  Include A/R Paid Report
                </label>
                <p className="text-xs text-slate-500 dark:text-zinc-400">
                  Includes shift payments received against customer accounts alongside standard transactions.
                </p>
              </div>
            </div>
            ...
            <label className="relative inline-flex items-center cursor-pointer">
              <input
                id="ar-paid-toggle"
                type="checkbox"
                checked={includeArPaidReport}
                onChange={(e) => setIncludeArPaidReport(e.target.checked)}
                disabled={loading}
