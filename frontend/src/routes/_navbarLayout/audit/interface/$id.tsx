...
function RouteComponent() {
  ...
  return (
    <div className="p-4">
      ...
      {/* Period Navigation with clickable picker */}
      ...
      {!loading && items.length > 0 ? (
        <div className="grid gap-3">
          {items.map((it) => (
            <div key={it._id} className="border rounded-lg p-3 bg-gray-50 shadow-sm">
              <div className="font-medium">{it.item}</div>
              <div className="text-sm text-gray-600">Status: {it.currentIssueStatus || "N/A"}</div>
            </div>
          ))}
        </div>
      ) : (
        !loading && <div className="text-gray-500">No items found for this selection.</div>
      )}
    </div>
  );
}
