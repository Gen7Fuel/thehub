...
function RouteComponent() {
  ...
  const handlePostNote = async () => {
    ...
    try {
      const token = localStorage.getItem("token");
      await axios.post(
        "/api/cycle-count/instance-notes",
        {
          instanceId,
          note: newNoteText,
        },
  ...
  // 🚀 Dedicated Admin Force-Sync Handler Function
  const handleFinalizeAndSync = async () => {
    if (!date || !site) return;

    const selectedDateStr = date.toISOString().split("T")[0];
    const confirmation = window.confirm(
      `This will create tickets for all the items which have been counted on the hub. Proceed with generating ticket of the count?`,
    );
    ...
    try {
      const response = await axios.post(
        ...
        {
          siteName: site,
          targetDate: selectedDateStr, // 💡 Pass the dynamically picked date parameter configuration
        },
        {
          headers: {
      ...
      alert(
        err.response?.data?.message ||
          "❌ CRITICAL Exception handling backend distribution tasks.",
      );
    } finally {
      setSyncing(false);
    }
  ...
  return (
    <>
            ...
            <div className="w-full max-w-full bg-white rounded-2xl border border-slate-200 shadow-xs overflow-hidden mb-8">
              <div className="p-4 bg-slate-50 border-b border-slate-200/80 flex items-center gap-2">
                ...
                <h2 className="text-xs font-black text-slate-800 uppercase tracking-wider">
                  Reconciled Core Matrix ({completedRecords.length})
                </h2>
              </div>
              ...
              <div className="overflow-x-auto w-full max-h-[60vh] block clear-both style-scrollbar">
                <table className="w-full text-left border-collapse text-sm table-fixed min-w-[1100px] md:min-w-full">
                  ...
                  {!loading &&
                    Object.entries(groupedCompleted).map(
                      ([categoryName, categoryRows]) => {
                        ...
                        return (
                          ...
                          >
                            <tr
                              ...
                              className="bg-slate-50/80 hover:bg-slate-100/70 cursor-pointer select-none transition-colors border-y border-slate-200/60"
                            >
                              <td
                                colSpan={9}
                                className="p-2.5 font-bold text-slate-700 text-[11px] uppercase tracking-wide sticky left-0 z-20 bg-slate-50/80"
                              >
            ...
            <div className="w-full max-w-full bg-white rounded-2xl border border-rose-200 shadow-2xs overflow-hidden">
              <div className="p-4 bg-rose-50/40 border-b border-rose-100/80 flex items-center gap-2">
                ...
                <h2 className="text-xs font-black text-rose-900 uppercase tracking-wider">
                  Incomplete Counts ({uncompletedRecords.length})
                </h2>
              </div>
              ...
              <div className="overflow-x-auto w-full max-h-[45vh] block clear-both style-scrollbar">
                <table className="w-full text-left border-collapse text-sm table-fixed min-w-[1100px] md:min-w-full">
                  <thead className="bg-rose-50/20 text-rose-800 font-bold sticky top-0 z-30 shadow-[0_1px_0_0_rgba(225,29,72,0.06)] uppercase tracking-wider text-[10px]">
                    <tr>
                      <th className="p-3 sticky left-0 bg-[#fff9f9] z-40 text-center w-14 border-r border-rose-100/50">
                        Image
                      </th>
                      <th className="p-3 sticky left-[56px] bg-[#fff9f9] z-40 w-36 font-mono border-r border-rose-100/50">
                        UPC
                      </th>
                      <th className="p-3 sticky left-[200px] bg-[#fff9f9] z-40 w-54 shadow-[4px_0_8px_-3px_rgba(0,0,0,0.03)] border-r border-rose-100/50">
                        Description
                      </th>
                      <th className="p-3 text-center w-28">FOH Count</th>
                      <th className="p-3 text-center w-28">BOH Count</th>
                      <th className="p-3 text-center w-24 border-x border-rose-100/30">
                        Total Pack
                      </th>
                      <th className="p-3 text-center w-28">Expected (CSO)</th>
                      <th className="p-3 text-center w-28">Variance (Pcs)</th>
                  ...
                  <tbody>
                    ...
                    {!loading && uncompletedRecords.length === 0 && (
                      <tr>
                        ...
                        >
                          All scheduled items for this date range have
                          successfully finalized core updates.
                        </td>
                      </tr>
                    )}
                  </tbody>
          ...
          <div className="p-5 max-h-[60vh] overflow-y-auto space-y-3.5 bg-slate-50/30">
            {notes.length === 0 ? (
              ...
              notes.map((note) => (
                <div
                  key={note.id}
                  className="bg-white p-4 rounded-xl border border-slate-200/60 shadow-3xs flex flex-col gap-1 text-left"
                >
                  ...
                  <p className="text-xs text-slate-600 leading-relaxed font-medium font-sans whitespace-pre-wrap">
                    {note.note}
                  </p>
                </div>
              ))
      ...
      <Dialog
        open={!!activeBarcodeItem}
        onOpenChange={(open) => {
          if (!open) setActiveBarcodeItem(null);
        }}
      >
        <DialogContent className="sm:max-w-md rounded-3xl overflow-hidden p-0 border-none bg-white">
          ...
          <div className="flex flex-col justify-center items-center p-8 pt-6">
            ...
            <div className="text-center px-4 w-full">
              <h3 className="text-sm font-black text-slate-900 mb-2 truncate">
            ...
            <button
              ...
              className="mt-6 w-full py-3 bg-slate-900 text-white rounded-2xl font-bold text-xs"
            >
              Close
            </button>
          </div>
