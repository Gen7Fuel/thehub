...
export function RouteComponent() {
  ...
  const [activeDialogRow, setActiveDialogRow] = useState<CarrierFCSRow | null>(
    null,
  );
  const [dialogInputValue, setDialogInputValue] = useState("");

  const [wizardStep, setWizardStep] = useState<"closed" | "warning" | "form">(
    "closed",
  );
  ...
  const fetchFCSData = async () => {
    try {
      ...
      alert(
        "Could not load the fuel surcharge rates. Please refresh the page.",
      );
    } finally {
      setLoading(false);
    }
  ...
  const sortedAndFilteredData = useMemo(() => {
    ...
    const filtered = data.filter((row) => {
      ...
      const carrierNormalized = (row["Carrier"] || "")
        .replace(/[-\s]/g, "")
        .toUpperCase();
  ...
  const uniqueCarriers = useMemo(
    () => Array.from(new Set(data.map((r) => r["Carrier"].trim()))).sort(),
    [data],
  );
  ...
  const openEditDialog = (row: CarrierFCSRow) => {
    ...
    //   ? row['Stg_FCS']
    //   : (row['Live_FCS'] ?? 0)
    // If Stg_Updated_At is present, a change is scheduled. Use Stg_FCS; otherwise default to Live_FCS.
    const baselineValue =
      row["Stg_FCS"] !== null && row["Stg_Updated_At"] !== null
        ? row["Stg_FCS"]
        : (row["Live_FCS"] ?? 0);

    setDialogInputValue(
      editedRows[key] !== undefined
        ? editedRows[key].toString()
  ...
  const handleAddEntryToStagingList = () => {
    ...
    const isStagedDuplicate = newEntriesList.some(
      (e) =>
        e.carrier.trim().toUpperCase() === currentCarrier &&
        e.province.trim().toUpperCase() === currentProvince,
    );

    const isDatabaseDuplicate = data.some(
      (r) =>
        r["Carrier"].trim().toUpperCase() === currentCarrier &&
  ...
  const handlePushNewEntriesToServer = async () => {
    ...
    try {
      const res = await axios.post(
        "/api/fuel-settings/carrier-fcs/batch",
      ...
      alert(
        "Something went wrong on the server. Could not create the records.",
      );
    }
  };
  ...
  const handlePushUpdatesBatch = async (isImmediateAction: boolean) => {
    ...
    try {
      ...
      if (res.status === 200) {
        alert(
          isImmediateAction
            ? "Changes applied live right now!"
            : "Changes saved and scheduled successfully.",
        );
  ...
  const totalStagedCount =
    Object.keys(editedRows).length +
    Object.keys(deletedRows).filter((k) => deletedRows[k]).length;
  ...
  if (loading) {
    return (
      <div className="p-6 text-sm font-medium text-gray-500 animate-pulse">
        Loading fuel surcharge information...
      </div>
    );
  }
      ...
      <div className="flex items-end justify-between w-full">
        <div>
          <h1 className="text-xl font-bold text-gray-900">
            Carrier Fuel Surcharges (FSC)
          </h1>
          <p className="text-xs text-gray-500 mt-1">
            Manage fuel surcharge rates for different carriers.{" "}
            {canEdit && (
              <span>
                You can make updates live right away, or schedule them to go
                live automatically on the 1st of next month.
              </span>
            )}
          </p>
        </div>
        ...
        <table className="w-full text-left border-collapse bg-white">
          <thead className="bg-gray-50/70 text-gray-700 text-xs font-semibold uppercase tracking-wider sticky top-0 border-b z-10">
            <tr>
              ...
              <th className="p-3 text-right bg-emerald-50/40 text-emerald-900 border-x">
                Current FSC ($)
              </th>
              <th className="p-4">Last Updated At</th>
              <th className="p-3 text-right bg-amber-50/40 text-amber-900 border-r">
                Scheduled FSC ($)
              </th>
          ...
          <tbody className="divide-y divide-gray-100 text-sm">
            {sortedAndFilteredData.length === 0 ? (
              ...
              sortedAndFilteredData.map((row) => {
                ...
                const isCommittedScheduleActive =
                  hasValidStagingMonth && hasSurchargesDiff;

                const liveValue =
                  row["Live_FCS"] !== null ? row["Live_FCS"] : 0;
                const committedStagedValue = isCommittedScheduleActive
                  ? row["Stg_FCS"]
                  : null;

                let rowClassName =
                  "hover:bg-gray-50/50 text-gray-700 transition-colors";
                if (isStagedDeleted) {
                  rowClassName =
                    "bg-red-50/60 text-gray-400 line-through select-none transition-colors";
                } else if (hasUnsavedLocalEdit) {
                  rowClassName =
                    "bg-amber-50/40 hover:bg-amber-50/70 text-gray-900 transition-colors";
                } else if (isCommittedScheduleActive) {
                  rowClassName =
                    "bg-blue-50/20 hover:bg-blue-50/40 text-gray-900 transition-colors";
                }
                ...
                return (
                  <tr key={key} className={rowClassName}>
                    ...
                    <td
                      className={`p-3 text-right font-mono font-bold bg-amber-50/10 border-r ${isCommittedScheduleActive ? "text-blue-600 font-semibold" : "text-gray-400"}`}
                    >
                      {committedStagedValue !== null
                        ? committedStagedValue.toFixed(4)
                        : "-"}
                    </td>
                    <td
                      className={`p-4 text-xs font-medium border-r ${isCommittedScheduleActive ? "text-blue-500 font-semibold" : "text-gray-400"}`}
                    >
                    ...
                    <td className="p-4 text-center whitespace-nowrap">
                      ...
                      {hasUnsavedLocalEdit && !isStagedDeleted && (
                        <span className="inline-flex px-2 py-0.5 rounded text-[10px] font-bold uppercase bg-amber-100 text-amber-700 border border-amber-200 animate-pulse">
                          Changing: (
                          {(committedStagedValue ?? liveValue).toFixed(4)} →{" "}
                          {editedRows[key].toFixed(4)})
                        </span>
                      )}
                      {!isStagedDeleted &&
                        !hasUnsavedLocalEdit &&
                        isCommittedScheduleActive && (
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-xs flex items-center justify-center z-50">
          <div className="bg-white border rounded-xl shadow-2xl w-full max-w-md p-6">
            ...
            <div className="space-y-2 mb-6 bg-gray-50 p-4 rounded-lg text-xs text-gray-600 border">
              <div>
                <span className="font-semibold text-gray-500">Carrier:</span>{" "}
                {activeDialogRow["Carrier"]}
              </div>
              <div>
                <span className="font-semibold text-gray-500">Province:</span>{" "}
                {activeDialogRow["Province"]}
              </div>
            </div>
            <div className="mb-6">
              <label className="block text-sm font-medium text-gray-700 mb-2">
                Enter New Number Amount
              </label>
              <input
                type="number"
                step="0.0001"
            ...
            <div className="flex justify-end gap-3">
              <button
                onClick={() => setIsEditDialogOpen(false)}
                className="px-4 py-2 border text-gray-700 rounded-lg text-sm"
              >
                Cancel
              </button>
              <button
          ...
          <div className="bg-white border rounded-xl shadow-2xl w-full max-w-md p-6 border-amber-200">
            <div className="flex items-center gap-3 text-amber-600 mb-4">
              ...
              <h3 className="text-lg font-bold text-gray-900">
                Please Check Bookworks First
              </h3>
            </div>
            <p className="text-sm text-gray-600 leading-relaxed mb-6">
              Before adding a new fuel surcharge rate to this dashboard, please
              double-check that you have already registered and added this
              trucking company inside the **Bookworks** system.
            </p>
          ...
          <div className="bg-white border rounded-xl shadow-2xl w-full max-w-2xl p-6 flex flex-col max-h-[85vh]">
            ...
            <div className="bg-gray-50/70 p-4 border rounded-xl grid grid-cols-2 gap-4 mb-4 text-xs">
              <div>
                ...
                <CreatableSelect
                  ...
                  onCreateOption={(val) =>
                    setFormCarrier(val.trim().toUpperCase())
                  }
                  value={
                    formCarrier
                      ? { value: formCarrier, label: formCarrier }
                      : null
                  }
                />
              </div>
              ...
              <div className="col-span-2">
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    value={formFCSValue}
                    onChange={(e) => setFormFCSValue(e.target.value)}
                  />
                  <button
            ...
            <div className="flex-1 overflow-auto border rounded-xl p-2 bg-gray-50/30 flex flex-col min-h-[150px] mb-6">
              <span className="text-xs font-bold uppercase tracking-wider text-gray-500 px-2 py-1 flex items-center gap-1.5 mb-2">
                <FileText className="w-3.5 h-3.5" /> Items Waiting to be Added (
                {newEntriesList.length})
              </span>

              {newEntriesList.length === 0 ? (
                <div className="flex-1 flex items-center justify-center text-xs text-gray-400 font-medium italic">
                  No items added to the temporary queue yet.
                </div>
              ) : (
            ...
            <div className="flex justify-between items-center border-t pt-4">
              <button
                onClick={closeCreationWizard}
                className="px-4 py-2 border text-gray-600 hover:bg-gray-50 rounded-lg text-sm font-medium"
              >
                Cancel
              </button>
              <button
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-xs flex items-center justify-center z-50">
          <div className="bg-white border rounded-xl shadow-2xl w-full max-w-md p-6 border-amber-200">
            <div className="flex items-center gap-3 text-amber-500 mb-4">
              ...
              <h3 className="text-lg font-bold text-gray-900">
                Confirm Future Scheduled Update
              </h3>
            </div>
            ...
            <p className="text-sm text-gray-600 leading-relaxed mb-4">
              ...
              <strong className="text-gray-900">
                {totalStagedCount} changes
              </strong>{" "}
              to be saved into the future planning table.
            </p>
            ...
            {Object.keys(deletedRows).filter((k) => deletedRows[k]).length >
              ...
              <div className="flex items-start gap-2 bg-red-50 text-red-900 border border-red-200 p-3 rounded-lg text-xs leading-relaxed mb-4 font-medium">
                ...
                <div>
                  <span className="font-bold uppercase tracking-wider block text-red-700 text-[10px] mb-0.5">
                    Live Deletion Warning
                  </span>
                  Your list includes row deletion requests. Deleting an item
                  completely skips scheduling and will happen **immediately live
                  right now** once you click confirm below.
                </div>
              </div>
            )}
            ...
            <div className="bg-amber-50/50 text-amber-900 border border-amber-200/60 p-3 rounded-lg text-xs leading-relaxed mb-6 font-medium">
              Note: The surcharge updates you made will stay hidden in the
              background for now. They will turn on automatically on live
              stations on **1st of {scheduledMonthName}**.
            </div>
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-xs flex items-center justify-center z-50">
          <div className="bg-white border rounded-xl shadow-2xl w-full max-w-md p-6 border-red-200">
            ...
            <p className="text-sm text-gray-600 leading-relaxed mb-6">
              ...
              directly to live calculation table. This action is{" "}
              <strong>immediate</strong> and cannot be undone.
            </p>
