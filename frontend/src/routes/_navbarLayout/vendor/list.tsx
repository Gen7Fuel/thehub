...
interface GroupedVendor {
  ...
  order_placement_method?: string;
  vendor_order_frequencies?: (number | "")[]; // ← allow array of numbers or empty strings
  locations: { site: string; frequency: number | ""; leadTime?: number | null }[];
    ...
    const fetchVendors = async () => {
      ...
      try {
        ...
        if (res.ok) {
          ...
          const grouped: GroupedVendor[] = Object.values(
            data.reduce((acc: Record<string, GroupedVendor>, v) => {
              const loc = { site: (v as any).location, frequency: (v as any).vendor_order_frequency || "", leadTime: (v as any).leadTime ?? null };
              if (!acc[v.name]) {
                acc[v.name] = {
                  _id: v._id,
                  name: v.name,
          ...
          // compute lead time average per grouped vendor (ignore 0 or null)
          grouped.forEach(g => {
            const times = g.locations.map(l => (typeof l.leadTime === "number" && l.leadTime > 0 ? l.leadTime : null)).filter(Boolean) as number[];
            if (times.length === 0) g.leadTimeAvg = null;
            else {
              const sum = times.reduce((s, n) => s + n, 0);
              g.leadTimeAvg = Math.round(sum / times.length);
            }
          });
              ...
              <table className="min-w-full border text-sm">
                <thead>
                  <tr className="bg-muted">
                    ...
                    <th className="p-2 border">Order Placement Method</th>
                    <th className="p-2 border text-center">Assigned</th>
                    <th className="p-2 border">Avg. Lead Time</th>
                    <th className="p-2 border">Actions</th>
                  </tr>
                </thead>
                <tbody>
                  {vendors.map((vendor: any) => (
                    <tr key={vendor._id} className="hover:bg-accent">
                            ...
                            <ul className="list-disc list-inside mt-2 space-y-1">
                              {selectedVendor?.locations.map((loc, idx) => (
                                <li key={idx}>
                                  {loc.site} {loc.frequency !== "" ? `- ${loc.frequency} week(s)` : ""}
                                  {" · "}
                                  Lead time: {typeof loc.leadTime === "number" && loc.leadTime > 0 ? `${loc.leadTime.toFixed(0)} day(s)` : "NA"}
                                </li>
                              ))}
                            </ul>
                          </DialogContent>
                      ...
                      <td className="p-2 border font-medium text-center">
                        {vendor.leadTimeAvg ? `${vendor.leadTimeAvg} day(s)` : "NA"}
                      </td>
                      <td className="p-2 border text-center">
                        <Button
                          variant="outline"
